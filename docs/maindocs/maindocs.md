...
::: tip
关于连接助理的绝大多数问题均可以通过重新加入大厅或重启 BakaXL 解决
:::

#### 一些 QA

- 为什么要推出连接助理？
- - 连接助理是为使用中继功能时出现超时的用户的用户所准备的。部分用户在连接时，游戏内会显示 `连接超时` ，因此连接助理会在玩家进入游戏连接前先提前做好连接。
  - *注: 连接助理不会优化您的网络环境也不会帮您自动联机，当您建立连接时耗时过长，并且您的 Minecraft 显示`连接超时`而导致联机失败时，您便可尝试使用连接助理*
- 如何使用连接助理?
- - 在您启动好游戏后，进入联机好友的大厅，点击 `让 BakaXL 帮助我建立连接` ，然后在游戏的 `多人游戏` 中进入指定局域网世界
