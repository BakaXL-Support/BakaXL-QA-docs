...
见 [BakaXL 常见错误代码](.\error_code.md) 一文
...
> BakaXL 目前支持 CurseForge 、 MCBBSv2 版本的整合包以及带有启动器的 Minecraft 游戏压缩包
...
将选项`Beta版:使用 Unicode UTF-8 提供全球语言支持(U)`旁的复选框取消勾选后重新启动即可关闭 Unicode UTF-8 模式
...
> 虽然以上 Mod 已被归入无法联机的列表，但是您依旧可以尝试进行联机，但是出现符合条件的错误时可以方便您快速排除错误原因


### 在使用联机助手时出现了问题

::: danger
由于联机助手处于测试阶段性功能，因此可能会出现大量问题

联机助手被推送至正式版的原因是由于 BakaXL Network 的变更导致了原有的正式版已无法使用 BakaXL Network 因此被迫推送至正式版保持领域服务不被中止
:::

::: tip
关于联机助理的绝大多数问题均可以通过重新加入大厅或重启 BakaXL 解决
:::

#### 一些 QA

- 为什么要推出联机助理？
- - 因为部分用户由于在使用中继功能中出现了 `已通过中继连接` 但是仍旧无法连接到房主的房间情况，联机助理便是为此而生的 
  - *注: 联机助理是为使用中继功能时出现超时的用户的用户所准备的，它不会优化您的网络环境也不会帮您自动联机，当您在通过中继传输流量时由于耗时过长导致您的Minecraft显示`连接超时`而导致联机失败时您便可尝试使用联机助理*
- 如何使用联机助理?
- - 在您启动好游戏后联机好友的大厅，如果您需要通过中继连接但是中继的耗时又导致您的游戏连接超时您便可使用联机助理
  - 方法: 点击联机助理，等待连接建立，在游戏的多人游戏中进入指定局域网广播
- 为什么要推出未经测试的功能?
- - 因为正式版已经无法使用 Network，因此需要更新至最新版才能使用 BakaXL Network，但是由于人手原因 BakaXL Network 新增的联机助理无法进行完整的测试因此导致了被迫更新正式版



#### 在使用联机助理时取消登录后无法再次重新建立连接

重启 BakaXL 即可解决此问题



#### 在联机助理通知完成后点击 "好" 后连接被中断并且无法重新建立连接
...
如果你仍能够看见普通的大厅尝试通过普通大厅来加入游戏，因为你的网络质量符合条件
